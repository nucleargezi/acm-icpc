...
NAME MeIoN_is_UMP45() {
    ...
    meion check = [&](int val) -> void {
        if (s_cmd.empty() or s_cmd.back() != val) {
            fail();
        }
        s_cmd.pop_back();
    };
    ...
    while (gc(c), c != EOF) {
        if (c == '[') {
            ...
            if (cmd == "h1") {
                ...
                if (not revs) {
                    out += "[";
                    s_info.emplace_back(cmd.substr(4));
                    s_cmd.emplace_back(val_url);
                } else {
                    out += "](" + s_info.back() + ")";
                    s_info.pop_back();
                    check(val_url);
                ...
                while (gc(c), c != EOF) {
                    buffer += c;
                    if (buffer.size() > 7 and
                        buffer.substr(buffer.length() - 8) == "[/quote]") {
                        break;
                    }
                }
                if (not(buffer.size() > 7 and
                        buffer.substr(buffer.length() - 8) == "[/quote]")) {
                    fail();
                }
                buffer.erase(buffer.length() - 8);
                while (not buffer.empty() and
                       (buffer.back() == '\n' or buffer.back() =='\r')) {
                    buffer.pop_back();
                }
                if (out.back() != '\n' and out.back() != '\r') {
